...
GROUP BY teams.name
ORDER BY COUNT(games.winner_id) DESC
LIMIT 1;
")"
...
echo "$($PSQL "SELECT DISTINCT games.year, teams.name
FROM teams
JOIN games ON teams.team_id = games.winner_id
AND games.round = 'Final'
GROUP BY teams.name, games.year
